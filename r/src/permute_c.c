             ...
             int *nl_in, int *lai, int *loi, double *l_weights) // number of locations to write and the locations
{
    ...
    // Clamp sigma to avoid degenerate kernels
    ...
    for (int x = 0; x <= nkx/2; x++) { // x is dist from center of gaussian
        double val = exp(-(x * x) / (2 * sigma * sigma));
        gaussian[kernel_midpoint + x] = val;
        gaussian[kernel_midpoint - x] = val;
    }
    ...
    // Compute 2D kernel from 1D kernel
    for (int i = 0; i < nky; i++) { // loop over kernel Y
        ...
        for (int j = 0; j < nkx; j++) { // loop over kernel X
            int xs = j - (nkx + 1) / 2;
            double ks = gaussian[i] * gaussian[j]; // 2d kernel from multiplying the 1d kernels
...
SEXP read_footprint() {
    ...
    for (int i=0; i<len; ++i) {
        double val = footprint[i];
        //Sanity check to catch any unexpected NaN or Inf
        if (!isfinite(val)) {
            UNPROTECT(1);
            error("Footprint contains non-finite values (NaN or Inf). Aborting read.");
        }
        REAL(out)[i] = val;
        //sum += val;
    }
