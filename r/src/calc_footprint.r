...
calc_footprint <- function(p, output = NULL, r_run_time,
  ...
  # save(p, output, r_run_time, projection, smooth_factor, time_integrate, xmn, xmx, xres, ymn, ymx, yres, file="/home/airtracker/airtracker/air-tracker-stiltctl/RData")
  ...
  # Assert kernel$rtime is ascending
  if (any(diff(kernel$rtime) < 0)) {
    stop("kernel$rtime must be ascending")
  }
  ...
  ## Perf Note: Takes ~250ms, but not always???
