...
do_run <- function(arg_list) {
    #args <- list(...)

    arg_names <- list()
    arg_values <- list()
    for (arg in arg_list) {
        tmp <- strsplit(arg, split = "=")
        arg_values <- append(arg_values, tmp[[1]][1])
        arg_names <- append(arg_names, tmp[[1]][2])
    }

    args <- setNames(arg_names, arg_values)
